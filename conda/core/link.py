...
class UnlinkLinkTransaction:
    ...
    def _verify_individual_level(prefix_action_group):
        ...
        # parallelizable I/O-and-hash work. conda-tempo Phase-2 S6 showed
        # perfectly linear O(M) scaling at 0.76 ms / action for 4 KB files,
        # with each action writing its own uuid-named intermediate so no
        # shared-state mutation. Fan out on a ThreadPoolExecutor when the
        # user has opted in via ``context.verify_threads``; preserve the
        # single-threaded default to keep shipping behaviour unchanged.
    ...
    def _verify_individual_level_parallel(all_actions, max_workers):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            return [err for err in pool.map(_safe_verify, all_actions) if err is not None]
