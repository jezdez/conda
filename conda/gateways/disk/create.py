...
def _win_copy_file(src, dst):
    ...
    # CopyFileW keeps the data copy in kernel32 instead of bouncing through
    # Python's read/write loop. The destination is expected not to exist here.
...
def compile_multiple_pyc(
    ...
    try:
        ...
        for offset in range(0, len(pyc_pairs), chunk_size):
            ...
            # Windows wrapper scripts reject newline-containing arguments, so run
            # the paired compiler from temporary files instead of passing it via -c.
